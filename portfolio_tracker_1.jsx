...
// FX rates to USD — user can update these
  ...
  return (
    <div style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{
          padding: "18px 22px", borderBottom: "1px solid #2a2d35",
          display: "flex", justifyContent: "space-between", alignItems: "center",
        }}>
          <h3 style={{ margin: 0, fontSize: 16, fontWeight: 600, color: "#e8e9ed" }}>{title}</h3>
...
function Field({ label, children }) {
  return (
    <div style={{ marginBottom: 14 }}>
      <label style={{ display: "block", fontSize: 12, color: "#8b8fa3", marginBottom: 5, fontWeight: 500, letterSpacing: "0.3px" }}>
        {label}
      </label>
      {children}
    </div>
...
const inputStyle = {
  width: "100%", padding: "9px 12px", background: "#12141a",
  border: "1px solid #2a2d35", borderRadius: 7, color: "#e8e9ed",
...
const btnSecondary = {
  padding: "10px 20px", background: "#2a2d35", color: "#c0c3ce",
...
function AddTransactionModal({ open, onClose, onSave }) {
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      bucket: form.bucket, notes: form.notes,
    ...
    <Modal open={open} onClose={onClose} title="Add Transaction">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <Field label="Ticker / Symbol">
          <input style={inputStyle} placeholder="e.g. AAPL, BTC" value={form.ticker} onChange={e => set("ticker", e.target.value)} />
        </Field>
        <Field label="Date">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <Field label={`Quantity`}>
          <input style={inputStyle} type="number" step="any" placeholder="0" value={form.qty} onChange={e => set("qty", e.target.value)} />
        </Field>
        <Field label={`Price (${market?.currency})`}>
          <input style={inputStyle} type="number" step="any" placeholder="0.00" value={form.price} onChange={e => set("price", e.target.value)} />
        </Field>
      </div>
      <Field label="Bucket">
        <select style={selectStyle} value={form.bucket} onChange={e => set("bucket", e.target.value)}>
          {BUCKET_OPTIONS.map(b => <option key={b} value={b}>{b}</option>)}
        </select>
      </Field>
      <Field label="Notes (optional)">
        <input style={inputStyle} placeholder="Entry thesis, target, stop-loss..." value={form.notes} onChange={e => set("notes", e.target.value)} />
      </Field>
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
        <button style={btnSecondary} onClick={onClose}>Cancel</button>
        <button style={btnPrimary} onClick={handleSave}>Save Transaction</button>
      </div>
    </Modal>
  );
}
...
// ─── Add Top-Up Modal ───
function AddTopupModal({ open, onClose, onSave }) {
  const [form, setForm] = useState({
  ...
  return (
    <Modal open={open} onClose={onClose} title="Wallet Deposit / Withdrawal">
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <Field label="Market">
          <select style={selectStyle} value={form.market} onChange={e => set("market", e.target.value)}>
            {MARKETS.map(m => <option key={m.id} value={m.id}>{m.flag} {m.name}</option>)}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <Field label={`Amount (${market?.currency})`}>
          <input style={inputStyle} type="number" step="any" placeholder="0.00" value={form.amount} onChange={e => set("amount", e.target.value)} />
        </Field>
        <Field label="Date">
          <input style={inputStyle} type="date" value={form.date} onChange={e => set("date", e.target.value)} />
        </Field>
      ...
      <Field label="Notes (optional)">
        <input style={inputStyle} placeholder="Source, purpose..." value={form.notes} onChange={e => set("notes", e.target.value)} />
      </Field>
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
        <button style={btnSecondary} onClick={onClose}>Cancel</button>
...
// ─── Update Price Modal ───
function UpdatePriceModal({ open, onClose, holdings, currentPrices, onSave }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Update Current Prices">
      <p style={{ fontSize: 13, color: "#8b8fa3", marginTop: 0, marginBottom: 16 }}>
      ...
      {holdings.map(h => {
        ...
        return (
          <div key={key} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
            ...
            <span style={{ width: 80, fontWeight: 600, color: "#e8e9ed", fontSize: 14 }}>{h.ticker}</span>
            <input
            ...
            />
            <span style={{ fontSize: 12, color: "#6b7080" }}>{mkt?.currency}</span>
          </div>
        );
      })}
      {holdings.length === 0 && <p style={{ color: "#6b7080", fontSize: 13 }}>No open positions to price.</p>}
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 14 }}>
        <button style={btnSecondary} onClick={onClose}>Cancel</button>
        <button style={btnPrimary} onClick={handleSave}>Save Prices</button>
      </div>
    </Modal>
  );
}
...
// ─── FX Rates Modal ───
function FxRatesModal({ open, onClose, fxRates, onSave }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="FX Rates to USD">
      <p style={{ fontSize: 13, color: "#8b8fa3", marginTop: 0, marginBottom: 16 }}>
      ...
      {Object.entries(rates).filter(([k]) => k !== "USD").map(([cur, val]) => (
        <div key={cur} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          <span style={{ width: 55, fontWeight: 600, color: "#e8e9ed", fontSize: 14 }}>1 {cur}</span>
          <span style={{ color: "#6b7080" }}>=</span>
          <input style={{ ...inputStyle, width: 130 }} type="number" step="any" value={val}
            onChange={e => setRates(p => ({ ...p, [cur]: e.target.value }))} />
          <span style={{ fontSize: 12, color: "#6b7080" }}>USD</span>
        </div>
      ))}
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 14 }}>
        <button style={btnSecondary} onClick={onClose}>Cancel</button>
        <button style={btnPrimary} onClick={handleSave}>Save Rates</button>
      </div>
    </Modal>
  );
}
...
function ConfirmModal({ open, onClose, onConfirm, message }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Confirm">
      <p style={{ color: "#c0c3ce", fontSize: 14, marginTop: 0 }}>{message}</p>
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 14 }}>
        <button style={btnSecondary} onClick={onClose}>Cancel</button>
        <button style={{ ...btnPrimary, background: "#ef4444" }} onClick={onConfirm}>Delete</button>
      </div>
    </Modal>
  );
}
...
function StatCard({ label, value, sub, accent }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 12, color: "#6b7080", fontWeight: 500, marginBottom: 6, letterSpacing: "0.4px", textTransform: "uppercase" }}>{label}</div>
      <div style={{ fontSize: 22, fontWeight: 700, color: accent || "#e8e9ed", fontFamily: "'JetBrains Mono', monospace" }}>{value}</div>
      {sub && <div style={{ fontSize: 12, color: "#6b7080", marginTop: 4 }}>{sub}</div>}
    </div>
  );
}
...
export default function PortfolioTracker() {
  ...
  useEffect(() => {
    loadData().then(d => {
      if (d && d.transactions && d.transactions.length > 0) {
        ...
        // First load — seed with Dow Jones data
        setData(SEED_DATA);
        saveData(SEED_DATA);
      }
      setLoading(false);
  ...
  const analytics = useMemo(() => {
    ...
    // Holdings: aggregate buys - sells per ticker per market
    const holdingMap = {};
    txns.forEach(tx => {
    ...
    MARKETS.forEach(m => {
      ...
      // Cash available = deposits - withdrawals - cost of current holdings + proceeds from sells
      ...
      marketStats[m.id] = {
        ...
        cash: cashLocal,
        holdings: mHoldings.length,
        investedUSD: investedLocal * rate,
        valueUSD: valueLocal * rate,
        realizedUSD: realizedLocal * rate,
        unrealizedUSD: unrealizedLocal * rate,
        cashUSD: cashLocal * rate,
        walletUSD: walletBalance * rate,
        currency: m.currency,
      };
    });
    ...
    return {
      holdings, closed, marketStats,
      totalInvestedUSD, totalValueUSD, totalRealizedUSD, totalUnrealizedUSD,
      totalCashUSD, totalDepositsUSD, bucketAlloc,
    };
  }, [data]);

  if (loading) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", background: "#12141a", color: "#6b7080", fontFamily: "'DM Sans', sans-serif" }}>
        Loading portfolio...
      </div>
    );
  }
  ...
  return (
    <div style={{
      minHeight: "100vh", background: "#12141a", color: "#e8e9ed",
      ...
      {/* Header */}
      <div style={{
        padding: "16px 22px", borderBottom: "1px solid #1e2028",
        display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12,
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 22 }}>📊</span>
          <div>
            <h1 style={{ margin: 0, fontSize: 18, fontWeight: 700, letterSpacing: "-0.3px" }}>Portfolio Tracker</h1>
            <span style={{ fontSize: 11, color: "#6b7080" }}>EGX · ADX · US · Crypto</span>
          </div>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button style={{ ...btnSecondary, fontSize: 12, padding: "7px 14px" }} onClick={() => setShowFxModal(true)}>💱 FX Rates</button>
          <button style={{ ...btnSecondary, fontSize: 12, padding: "7px 14px" }} onClick={() => setShowPriceModal(true)}>📈 Update Prices</button>
          <button style={{ ...btnPrimary, fontSize: 12, padding: "7px 14px" }} onClick={() => setShowTopupModal(true)}>💰 Deposit</button>
          <button style={{ ...btnPrimary, fontSize: 12, padding: "7px 14px", background: "#22c55e" }} onClick={() => setShowTxModal(true)}>＋ Trade</button>
        </div>
      </div>

      {/* Tabs */}
      <div style={{
        display: "flex", gap: 0, borderBottom: "1px solid #1e2028",
        padding: "0 22px", overflowX: "auto",
      }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            padding: "12px 18px", background: "none", border: "none",
            borderBottom: tab === t.id ? "2px solid #3b82f6" : "2px solid transparent",
            color: tab === t.id ? "#e8e9ed" : "#6b7080",
        ...
        <div style={{ flex: 1 }} />
        <select style={{ ...selectStyle, width: "auto", fontSize: 12, padding: "6px 10px", margin: "6px 0", background: "#1a1d23" }}
        ...
        {tab === "dashboard" && (
          <>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 14, marginBottom: 24 }}>
              <StatCard label="Portfolio Value (USD)" value={`$${formatNum(portfolioValue)}`} />
              <StatCard label="Total Invested (USD)" value={`$${formatNum(analytics.totalInvestedUSD)}`} />
              <StatCard label="Total P&L" value={`${totalPL >= 0 ? "+" : ""}$${formatNum(totalPL)}`}
                accent={plColor}
                sub={analytics.totalInvestedUSD > 0 ? `${totalPL >= 0 ? "+" : ""}${formatNum((totalPL / analytics.totalInvestedUSD) * 100)}%` : ""} />
              <StatCard label="Cash Available" value={`$${formatNum(analytics.totalCashUSD)}`} accent="#f59e0b" />
            </div>

            {/* Market Breakdown */}
            <h3 style={{ fontSize: 14, fontWeight: 600, color: "#8b8fa3", marginBottom: 12, letterSpacing: "0.5px", textTransform: "uppercase" }}>Market Breakdown</h3>
                ...
                return (
                  <div key={m.id} style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                      ...
                      <span style={{ fontSize: 11, color: "#6b7080", marginLeft: "auto" }}>{m.currency}</span>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      {[
                        ...
                        <div key={k}>
                          <div style={{ fontSize: 11, color: "#6b7080" }}>{k}</div>
                          <div style={{
                            fontSize: 14, fontWeight: 600,
                            color: k === "P&L" ? (mPL >= 0 ? "#22c55e" : "#ef4444") : "#e8e9ed",
                            fontFamily: "'JetBrains Mono', monospace",
                          }}>{v}</div>
                        </div>
                      ))}
                    </div>
                    <div style={{ marginTop: 10, fontSize: 11, color: "#6b7080" }}>{s.holdings} open position{s.holdings !== 1 ? "s" : ""}</div>
                  </div>
                );
              })}
            </div>
            ...
            {/* Bucket Allocation */}
            <h3 style={{ fontSize: 14, fontWeight: 600, color: "#8b8fa3", marginBottom: 12, letterSpacing: "0.5px", textTransform: "uppercase" }}>Strategy Allocation (USD)</h3>
              ...
              {BUCKET_OPTIONS.map((b, i) => {
                ...
                return (
                  <div key={b} style={{
                    ...
                    <div style={{ fontSize: 12, color: "#6b7080", marginBottom: 6 }}>{b}</div>
                    <div style={{ fontSize: 18, fontWeight: 700, color: colors[i], fontFamily: "'JetBrains Mono', monospace" }}>
                      ${formatNum(val)}
                    </div>
                    <div style={{ marginTop: 6, height: 4, background: "#2a2d35", borderRadius: 2, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${Math.min(pct, 100)}%`, background: colors[i], borderRadius: 2, transition: "width 0.3s" }} />
                    </div>
                    <div style={{ fontSize: 11, color: "#6b7080", marginTop: 4 }}>{formatNum(pct, 1)}% of portfolio</div>
                  </div>
                );
              })}
            </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: 13, fontWeight: 600, color: "#8b8fa3", marginBottom: 8 }}>📋 Copy Summary for Claude</div>
              <p style={{ fontSize: 12, color: "#6b7080", margin: "0 0 10px 0" }}>Click below to copy your portfolio summary, then paste it in your Investment Analysis Hub project chat.</p>
              <button style={{ ...btnPrimary, fontSize: 12, padding: "8px 16px" }} onClick={() => {
        ...
        {tab === "holdings" && (
          <>
            {analytics.holdings.filter(h => marketFilter === "all" || h.market === marketFilter).length === 0 ? (
              <div style={{ textAlign: "center", padding: "60px 20px", color: "#6b7080" }}>
                    ...
                    <tr>
                      {["Market", "Ticker", "Qty", "Avg Cost", "Current", "Value", "P&L", "P&L %", "Bucket"].map(h => (
                        <th key={h} style={{
                          ...
                          color: "#6b7080", fontWeight: 600, borderBottom: "1px solid #2a2d35",
                  ...
                  <tbody>
                    {analytics.holdings
                      ...
                      .map(h => {
                        const mkt = MARKETS.find(m => m.id === h.market);
                        return (
                          <tr key={`${h.market}:${h.ticker}`} style={{ borderBottom: "1px solid #1e2028" }}>
                            <td style={{ padding: "10px 12px", whiteSpace: "nowrap" }}>{mkt?.flag} {mkt?.id.toUpperCase()}</td>
                            <td style={{ padding: "10px 12px", fontWeight: 700, fontFamily: "'JetBrains Mono', monospace" }}>{h.ticker}</td>
                            <td style={{ padding: "10px 12px", fontFamily: "'JetBrains Mono', monospace" }}>{formatNum(h.qty, h.qty < 1 ? 6 : 2)}</td>
                            <td style={{ padding: "10px 12px", fontFamily: "'JetBrains Mono', monospace" }}>{formatNum(h.avgCost)}</td>
                            <td style={{ padding: "10px 12px", fontFamily: "'JetBrains Mono', monospace", color: h.currentPrice !== null ? "#e8e9ed" : "#6b7080" }}>
                            ...
                            <td style={{
                              ...
                              color: h.unrealizedPL !== null ? (h.unrealizedPL >= 0 ? "#22c55e" : "#ef4444") : "#6b7080",
                            }}>
                              {h.unrealizedPL !== null ? `${h.unrealizedPL >= 0 ? "+" : ""}${formatNum(h.unrealizedPL)}` : "—"}
                            </td>
                            <td style={{
                              padding: "10px 12px", fontFamily: "'JetBrains Mono', monospace", fontWeight: 600,
                              color: h.unrealizedPct !== null ? (h.unrealizedPct >= 0 ? "#22c55e" : "#ef4444") : "#6b7080",
                            }}>
                              {h.unrealizedPct !== null ? `${h.unrealizedPct >= 0 ? "+" : ""}${formatNum(h.unrealizedPct, 1)}%` : "—"}
                            </td>
                            <td style={{ padding: "10px 12px", fontSize: 12, color: "#8b8fa3" }}>{h.bucket}</td>
                          </tr>
                        );
                      })}
                  </tbody>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  ...
                  <tbody>
                    {[...filteredTx].sort((a, b) => new Date(b.date) - new Date(a.date)).map(tx => {
                      ...
                      return (
                        <tr key={tx.id} style={{ borderBottom: "1px solid #1e2028" }}>
                          <td style={{ padding: "10px 12px", whiteSpace: "nowrap", fontSize: 13 }}>{formatDate(tx.date)}</td>
                          ...
                          <td style={{ padding: "10px 12px", fontWeight: 700, fontFamily: "'JetBrains Mono', monospace" }}>{tx.ticker}</td>
                          <td style={{ padding: "10px 12px", fontFamily: "'JetBrains Mono', monospace" }}>{formatNum(tx.qty, tx.qty < 1 ? 6 : 2)}</td>
                          <td style={{ padding: "10px 12px", fontFamily: "'JetBrains Mono', monospace" }}>{formatNum(tx.price)}</td>
                          <td style={{ padding: "10px 12px", fontFamily: "'JetBrains Mono', monospace", fontWeight: 600 }}>
                            {formatNum(tx.qty * tx.price)} {tx.currency}
                          </td>
                          <td style={{ padding: "10px 12px", fontSize: 12, color: "#8b8fa3" }}>{tx.bucket}</td>
                          <td style={{ padding: "10px 12px", fontSize: 12, color: "#6b7080", maxWidth: 150, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{tx.notes}</td>
                          <td style={{ padding: "10px 12px" }}>
                            <button onClick={() => setDeleteTarget({ type: "tx", id: tx.id })} style={{
                              background: "none", border: "none", color: "#6b7080", cursor: "pointer", fontSize: 14, padding: "2px 6px",
                            }}>🗑</button>
                          </td>
                        </tr>
                      );
          ...
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 14, marginBottom: 24 }}>
              {MARKETS.filter(m => marketFilter === "all" || m.id === marketFilter).map(m => {
                ...
                return (
                  <div key={m.id} style={{
                    background: "#1a1d23", border: "1px solid #2a2d35", borderRadius: 10, padding: 16,
                  }}>
                    ...
                    {[
                      ...
                      <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: "1px solid #1e2028" }}>
                        <span style={{ fontSize: 13, color: "#8b8fa3" }}>{k}</span>
                        <span style={{ fontSize: 13, fontWeight: 600, fontFamily: "'JetBrains Mono', monospace" }}>{v} {m.currency}</span>
                      </div>
                    ))}
                  </div>
                );
            ...
            <h3 style={{ fontSize: 14, fontWeight: 600, color: "#8b8fa3", marginBottom: 12, letterSpacing: "0.5px", textTransform: "uppercase" }}>Deposit / Withdrawal History</h3>
            {filteredTopups.length === 0 ? (
              <div style={{ textAlign: "center", padding: "40px 20px", color: "#6b7080" }}>No wallet transactions yet.</div>
            ) : (
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                  ...
                  <tbody>
                    {[...filteredTopups].sort((a, b) => new Date(b.date) - new Date(a.date)).map(t => {
                      ...
                      return (
                        <tr key={t.id} style={{ borderBottom: "1px solid #1e2028" }}>
                          <td style={{ padding: "10px 12px", fontSize: 13 }}>{formatDate(t.date)}</td>
                          <td style={{ padding: "10px 12px" }}>{mkt?.flag} {mkt?.id.toUpperCase()}</td>
                          <td style={{ padding: "10px 12px" }}>
                            <span style={{
                              padding: "3px 8px", borderRadius: 4, fontSize: 11, fontWeight: 700,
                              background: t.type === "deposit" ? "rgba(59,130,246,0.15)" : "rgba(239,68,68,0.15)",
                              color: t.type === "deposit" ? "#3b82f6" : "#ef4444",
                            }}>
                              {t.type === "deposit" ? "⬆ DEPOSIT" : "⬇ WITHDRAWAL"}
                            </span>
                          </td>
                          <td style={{ padding: "10px 12px", fontWeight: 600, fontFamily: "'JetBrains Mono', monospace" }}>
                            {formatNum(t.amount)} {t.currency}
                          </td>
                          <td style={{ padding: "10px 12px", fontSize: 12, color: "#6b7080" }}>{t.notes}</td>
                          <td style={{ padding: "10px 12px" }}>
      ...
      {/* Modals */}
      <AddTransactionModal open={showTxModal} onClose={() => setShowTxModal(false)} onSave={addTransaction} />
      <AddTopupModal open={showTopupModal} onClose={() => setShowTopupModal(false)} onSave={addTopup} />
      <UpdatePriceModal open={showPriceModal} onClose={() => setShowPriceModal(false)}
        holdings={analytics.holdings} currentPrices={data.currentPrices} onSave={updatePrices} />
      <FxRatesModal open={showFxModal} onClose={() => setShowFxModal(false)} fxRates={data.fxRates} onSave={updateFx} />
      <ConfirmModal
        open={!!deleteTarget}
